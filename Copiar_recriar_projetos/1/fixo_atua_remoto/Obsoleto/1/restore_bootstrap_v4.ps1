...
# Versão: 1.1 - 2026-05-19 (com Windows Forms)
...
# ---------- NOVA LÓGICA: SELEÇÃO DO JSON VIA WINDOWS FORMS ----------
if (-not $JsonPath) {
    $guiOk = $true
    try {
        # Tenta carregar System.Windows.Forms (modo GUI) [web:2][web:4]
        ...
        # Se der erro (sem GUI, PowerShell 7 em Linux, etc.), cai para modo console
        $guiOk = $false
        Write-Log "Falha ao usar Windows Forms/OpenFileDialog: $($_.Exception.Message)" "WARNING"
        Write-Host "Não foi possível abrir a janela gráfica de seleção de arquivo." -ForegroundColor Yellow
        Write-Host "Voltando para entrada no console..." -ForegroundColor Yellow
