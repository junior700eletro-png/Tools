...
function Show-LargeFoldersMenu {
    ...
    Write-Host "  [1] IGNORAR pastas grandes (RECOMENDADO)" -ForegroundColor Green
    Write-Host "  [2] PROCESSAR TUDO (pode ser lento)" -ForegroundColor Yellow
    Write-Host ""
    $choice = Read-Host "Escolha uma opcao"
    if ($choice -eq "1") { return $true }
    elseif ($choice -eq "2") { return $false }
    else {
        Write-Host "Opcao invalida!" -ForegroundColor Red
...
function Get-FilesRecursive {
    ...
    foreach ($item in $items) {
        ...
        if ($item.PSIsContainer) {
            if (-not ($UseIgnore -and $IgnoreFolders -contains $item.Name)) {
                ...
                foreach ($subItem in $subItems) {
                    ...
                    if (-not $subItem.PSIsContainer) {
                        ...
                        if ($AllowedExtensions -contains $ext) {
                            $fileContent = Read-FileContent -FilePath $subItem.FullName
                            if ($null -ne $fileContent -and $fileContent.Length -lt 5MB) {
                                $relativePath = $subItem.FullName.Substring($Path.Length).TrimStart('\')
                                $files += @{"path" = $relativePath; "content" = $fileContent; "size" = $subItem.Length}
                                $fileCount++
                                Write-Host "  OK - $($subItem.Name) ($fileCount/$MaxFiles)" -ForegroundColor Green
                            }
                        }
                    }
                }
...
try {
    ...
    foreach ($folder in $allFolders) {
        ...
        if ($size -gt 50MB) {
            $largeFolders += @{Name = $folder.Name; Size = $size}
