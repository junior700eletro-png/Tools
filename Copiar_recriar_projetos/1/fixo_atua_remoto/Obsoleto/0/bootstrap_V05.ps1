            ...
            foreach ($item in $items) {
                ...
                if ($item.PSIsContainer) {
                    if ($UseIgnore -and $IgnoreFolders -contains $item.Name) {
                        $ignoredCount++
                        Write-Host "  IGNORANDO - $($item.Name)" -ForegroundColor Yellow
                    } else {
                    ...
                    if ($AllowedExtensions -contains $ext) {
                        try {
                            $content = Get-Content $item.FullName -Raw -Encoding UTF8 -ErrorAction SilentlyContinue
                            if ($null -ne $content -and $content.Length -lt 5MB) {
                                ...
                                $fileCount++
                                Write-Host "  OK - $relativePath ($fileCount/$MaxFiles)" -ForegroundColor Green
                            }
                        } catch {
                            Write-Host "  ERRO ao ler - $($item.FullName)" -ForegroundColor Red
                        }
                    }
                }
            }
...
try {
    ...
    foreach ($folder in $allFolders) {
        ...
        if ($size -gt 50MB) {
            ...
            $smallFolders += @{Name = $folder.Name; Size = $size; Path = $folder.FullName}
    ...
    if ($ignoreMode) {
        $foldersToCapture = @($allFolders | Where-Object {
            $folderName = $_.Name
            $ignoreFolders -notcontains $folderName
        })
    } else {
