...
# RECUPERA \\ A PARTIR DE ¦ ¦ ADPATA ONE
...
function Restore-BarrasInvertidas {
    param([string]$Content)
    # Converte ¦¦ de volta para \\ (4 barras)
    ...
    switch ($choice) {
        ...
        '3' {
            ...
            if (Test-Path $filePath) {
                ...
                $contentToSave = Restore-BarrasInvertidas -Content $contentOriginal
                ...
                Write-Log "Integridade: 100% preservada (barras restauradas)" "SUCCESS"
                $menuLoop = $false
