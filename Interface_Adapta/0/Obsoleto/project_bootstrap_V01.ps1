...
$projectData = @{
    "pastas" = @("src", "config", "logs", "output", "Obsoleto")
    "arquivos" = @(
        @{
            "caminho" = "src/communication_protocol.py"
            "conteudo" = @"
import re
import json
...
"@
        ...
        @{
            "caminho" = "src/command_executor.py"
            "conteudo" = @"
import os
import subprocess
import shutil
...
"@
        ...
        @{
            "caminho" = "src/main_orchestrator.py"
            "conteudo" = @"
import time
import threading
from datetime import datetime
...
Write-Host "1. Execute: python src/main_orchestrator.py" -ForegroundColor Cyan
